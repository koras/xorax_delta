...
local control = dofile(script_path .. "\\interface\\control.lua");
local words = dofile(script_path .. "\\langs\\words.lua");
...
local function initCandle(barCandleLocal)
    ...
    if start_init then
     ...
     --   loger.save('currentTime 1111 = '.. setting.datetime.hour.. ':' .. setting.datetime.min);
...
function setArrayCandles(setting, barCandle, numberCandle)
    ...
    if #setting.array_candle > 0 then
     ...
     --   loger.save('currentTime 2222 = '.. setting.datetime.hour.. ':' .. setting.datetime.min); 
...
function setBarCandle(setting, bar, collbackFunc)
   ...
   -- loger.save('currentTime 44444 = '.. setting.datetime.hour.. ':' .. setting.datetime.min); 

    calculateSignal(bar);
    if nil ~= collbackFunc then 
        collbackFunc(setting, bar);
    end
end
...
local function getSignal(setting, collbackFunc)
    ...
    while i >= 1 do
        ...
        if bars_temp[j - 1].datetime.hour ~= nil then
            ...
            setting.datetime = bars_temp[j - 1].datetime;
        --    loger.save('currentTime 333335 = '.. bars_temp[j - 1].datetime.hour.. ':' .. bars_temp[j - 1].datetime.min); 
