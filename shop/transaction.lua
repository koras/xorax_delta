...
local loger = dofile(script_path .. "\\engine\\loger.lua");
 ...
 function send(typeMarket, price, quantity, type, trans_id_buy, direction)
    ...
    loger.save("CLASSCODE = " .. tostring(Transaction.CLASSCODE));
    loger.save("SECCODE = " .. tostring(Transaction.SECCODE));
    loger.save("ACCOUNT = " .. tostring(Transaction.ACCOUNT));
    loger.save("SEC_CODE = " .. tostring(setting.SEC_CODE));
    loger.save("typeMarket = " .. tostring(typeMarket));
    loger.save("price = " .. tostring(price));
    loger.save("quantity= " .. tostring(quantity));
    loger.save("type= " .. tostring(type));
    loger.save("setting.type_instrument= " .. tostring(type));
    ...
    elseif type == "NEW_ORDER" then
        ...
        loger.save('       sendTransaction  NEW_ORDER      ');
        Transaction.PRICE = price;
    end

    loger.save('       sendTransaction        ');


    local res = sendTransaction(Transaction);
    loger.save('res asdasdasd ' .. tostring(res));

    if res ~= "" then
        loger.save('Transaction  ' .. res)
        message( res.. " "..setting.type_instrument )
        return nil, res
